...
Implementation of a (conditional, convolutional) variational autoencoder (VAE) in tensorflow.
Inspiration for this code taken from the following resources:
...
Variational inference replaces the Bayesian paradigm of marginalizing over latent variables for
prediction or inference with the calculation of derivatives (optimization) on the prior KL (unit normal)
...
respect to the latent space, with a penalty term (KL divergence) that constrains our approximation 
...
def Convolution(inputs, weights, biases, activation=tf.nn.relu, maxpool=False):
    ...
    # convolution = tf.add(tf.nn.conv2d(inputs, weights, strides=[1,1,1,1,1,1], padding='SAME'), biases)
    convolution = tf.add(tf.nn.conv2d(inputs, weights, strides=[1,1,1,1], padding='SAME'), biases)
...
class VAE():
    ...
    HYPERPARAMS = {
        ...
        "dropout_rate": 0.90,
        "num_classes": 0,
        "session_name": 'forward'
    }

    def __init__(self, hyperparameters={}, meta_graph=None, new_graph=True):
        """ 
        INPUTS:
        ...
        self.layer_names = list(range(len(self.blueprint)))
        self.sess = tf.Session()

        self.x = tf.placeholder(tf.float32, shape=[None, self.blueprint[0]], name='x')
        self.target = tf.placeholder(tf.float32, shape=[None, self.blueprint[0]], name='target')
        self.z = tf.placeholder(tf.float32, shape=[None, self.blueprint[-1]], name='z')
        ...
        if new_graph:
            ...
            self.new_graph = True
            with tf.variable_scope(self.session_name):
                self.Parameterize()
    ...
    def Parameterize(self):
        ...
        # ====================== NORMAL ENCODER PARAMETERIZATION ======================
            ...
            input2output = list(zip(self.blueprint[:-1], self.blueprint[1:], 
                                    self.layer_names[:-1], self.layer_names[1:]))
        ...
        input2output = list(zip(reversed(self.blueprint[1:]), reversed(self.blueprint[:-1]), 
                            decoder_names[:-1], decoder_names[1:]))
    ...
    def Build_Convolutions(self):
        ...
        e_weight_keys = []
        e_bias_keys = []
        layer_names = list(range(len(self.convolutions)+1))
        ...
        for theta in ('_mean', '_log_var'):
            ...
            # Affine transformation into Z:
            with tf.variable_scope('encoder_weights'):
                self.weights['encoder'][weight_name] = \
                    Xavier(weight_name, (256, self.blueprint[-1]))
                #Xavier(weight_name, (3*3*final_dim, self.blueprint[-1]))
            """
    ...
    def Encode(self):
        ...
        if not isinstance(self.convolutions, int):
            ...
            params = list(zip(sorted(weights.keys()), sorted(biases.keys())) )
    ...
    def Convolutional_Encoding(self):
        ...
        # Reshape flattened input to be the original pixel dimensions
        ...
        encoding = tf.reshape(encoding, [-1, 256])
        z_log_var = tf.add(tf.matmul(encoding, weights[params[-2][0]]), biases[params[-2][1]])
        z_mean = tf.add(tf.matmul(encoding, weights[params[-1][0]]), biases[params[-1][1]])
        return(z_mean, z_log_var)
    ...
    def Decode(self):
        ...
        output_mean = tf.add(tf.matmul(decoding, weights[params[-1][0]]), biases[params[-1][1]])
    ...
    def Optimize(self):
        ...
        with tf.name_scope('cost'):
            ...
            self.cost = tf.reduce_mean(self.L2_Loss() + self.KL_Divergence())
        ...
        with tf.name_scope("Adam_optimizer"):
            ...
            if not self.new_graph:
                ...
                method = tf.keras.optimizers.Adam(learning_rate=self.learning_rate,
                ...
                #clipped_gradients = [(tf.clip_by_value(delta, -1.0, 1.0), param) for delta,param in gradients]
                self.optimizer = method.apply_gradients(gradients)


    def Reconstruction_Loss(self, offset=1e-6):
    ...
    def L2_Loss(self):
        ...
        return(tf.reduce_sum(tf.square(self.output_mean - self.target), axis=1))
    ...
    def Fit(self, X, Y, labels=0):
        ...
        if isinstance(labels, int):
            ...
            feed = {self.x: X, self.target: Y}
            values = self.sess.run([self.optimizer, self.cost, self.z, self.output_mean], feed_dict=feed)
        else:

            feed = {self.labels: labels, self.x: X, self.target: Y}
            values = self.sess.run([self.optimizer, self.cost, self.z, self.output_mean], feed_dict=feed)
    ...
    def Reconstruct(self, X, labels=0):
        ...
        if isinstance(labels, int):
            values = self.sess.run([self.z, self.output_mean], feed_dict={self.x: X})
        else:
            values = self.sess.run([self.z, self.output_mean], feed_dict={self.x: X, self.labels: labels})
